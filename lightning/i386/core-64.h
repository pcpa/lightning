...
x86_finish(jit_state_t _jit, void *p0)
{
	...
	MOVBir(0, _RAX);
    assert(_jitl.nextarg_puti == 0 && _jitl.nextarg_putfp == 0);
    jit_calli(p0);
    if (_jitl.argssize) {
...
__jit_inline void
x86_muli_ul_(jit_state_t _jit,
	     jit_gpr_t r0, unsigned long i0)
{
    if (r0 == _RAX) {
	jit_movi_ul(_RDX, i0);
	MULQr(_RDX);
...
__jit_inline void
x86_btest_r64(jit_state_t _jit,
